...
let AuthService = class AuthService {
    ...
    async login(payloads, req, needTwoFactor = true) {
        ...
        if (content) {
            const { username: cUsername, password: cPassword, retries = 0, lastlogon, lastip, lastaddr, twoFactorActived, } = content;
            ...
            if (username === cUsername && password === cPassword) {
                ...
                (0, child_process_1.exec)(`notify "登陆通知" "你于${new Date(timestamp).toLocaleString()}在${address}登陆成功，ip地址${ip}"`);
                await this.getLoginLog();
                await this.insertDb({
                    type: 'loginLog',
                    info: { timestamp, address, ip, status: auth_1.LoginStatus.success },
                });
                return {
                    code: 200,
                ...
                (0, child_process_1.exec)(`notify "登陆通知" "你于${new Date(timestamp).toLocaleString()}在${address}登陆失败，ip地址${ip}"`);
                await this.getLoginLog();
                await this.insertDb({
                    type: 'loginLog',
                    info: { timestamp, address, ip, status: auth_1.LoginStatus.fail },
                });
